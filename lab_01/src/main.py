...
class Window():
    ...
    def createCanvas(self):
        ...
        for i in range(2):
            ...
            if i == 0:
                tk_widget.place(x = 400, y = 0, width = CANVAS_WIDTH, height = CANVAS_HEIGHT)
            else:
                tk_widget.place(x = 400, y = CANVAS_HEIGHT, width = CANVAS_WIDTH, height = CANVAS_HEIGHT)
        return canvases
    ...
    def createInterface(self):
        Label(text = "ИНТЕРВАЛ", font = ("Arial", 16, "bold"), bg = "#e0f0f0",
            fg = "purple").place(width = 400, height = 30, x = 0 , y = 20)

        Label(text = "Xmin\t\tXmax", font = ("Arial", 16), bg = "#e0e0eb",
            fg = "#29293d").place(width = 400, height = 20, x = 0, y = 70)

        xMinEntry = Entry(font = ("Arial", 17))
        xMinEntry.place(width = 180, height = 30, x = 10 , y = 100)
        xMinEntry.insert(0, X_MIN_INIT)

        xMaxEntry = Entry(font = ("Arial", 17))
        xMaxEntry.place(width = 175, height = 30, x = 10 + 190, y = 100)
        xMaxEntry.insert(0, X_MAX_INIT)

        Label(text = "РАВНОМЕРНОЕ РАСПРЕДЕЛЕНИЕ", font = ("Arial", 16, "bold"), bg = "#e0f0f0",
            fg = "purple").place(width = 400, height = 30, x = 0 , y = 160)

        Label(text = "a\t\tb", font = ("Arial", 16), bg = "#e0e0eb",
            fg = "#29293d").place(width = 400, height = 20, x = 0, y = 210)

        aEntry = Entry(font = ("Arial", 17))
        aEntry.place(width = 180, height = 30, x = 10, y = 240)

        bEntry = Entry(font = ("Arial", 17))
        bEntry.place(width = 175, height = 30, x = 10 + 190, y = 240)

        uniformDistrButton = Button(
            text = "Построить графики", font = ("Arial", 16), 
            highlightbackground = "#b3b3cc", highlightthickness = 30, fg = "purple",
            command = lambda: self.drawUniformDistrGraphs(aEntry, bEntry, xMinEntry, xMaxEntry))
        uniformDistrButton.place(width = 246, height = 26,  x = 0 + 80, y = 282)

        Label(text = "РАСПРЕДЕЛЕНИЕ ЭРЛАНГА", font = ("Arial", 16, "bold"), bg = "#e0f0f0",
            fg = "purple").place(width = 400, height = 30, x = 0 , y = 340)

        Label(text = "k\t\tλ", font = ("Arial", 16), bg = "#e0e0eb",
            fg = "#29293d").place(width = 400, height = 20, x = 0, y = 390)

        kEntry = Entry(font = ("Arial", 17))
        kEntry.place(width = 180, height = 30, x = 10, y = 420)

        lEntry = Entry(font = ("Arial", 17))
        lEntry.place(width = 175, height = 30, x = 10 + 190, y = 420)

        erlangDistrButton = Button(
            text = "Построить графики", font = ("Arial", 16), 
            highlightbackground = "#b3b3cc", highlightthickness = 30, fg = "purple",
            command = lambda: self.drawErlangDistrGraphs(kEntry, lEntry, xMinEntry, xMaxEntry))
        erlangDistrButton.place(width = 246, height = 26,  x = 0 + 80, y = 462)

        infoButton = Button(
            text = "Информация о программе", font = ("Arial", 16), 
            highlightbackground = "#a3a3a3", highlightthickness = 30, fg = "#33334d",
            command = lambda: self.aboutProgram())
        infoButton.place(width = 260, height = 26,  x = 0 + 80, y = 760)
